...
def calculate_returns(rewards, gamma):
    ...
    for t in range(T): 
        returns[t] = np.sum([gamma**k * rewards[t + k] for k in range(T - t)])
...
def value_loss(target, prediction):
    ...
    """
    arr = tf.zeros(tf.shape(target), dtype=tf.float32)
    arr = tf.reduce_mean((target-prediction)**2)
    return arr
...
def policy_loss(pi_a, pi_old_a, advantage, epsilon):
    ...
    u_i = pi_a/pi_old_a
    clipped = np.clip(u_i,1-epsilon,1+epsilon)
    minimum = tf.minimum(u_i * advantage,clipped * advantage)
    loss = - np.mean(minimum)
    return loss
        ...
        for epoch in range(K):
            for batch in dataset:
                ...
                with tf.GradientTape() as tape:
                    pi_a = activations.softmax(policy_network.policy(observation))
                ...
                gradient_policy = tape.gradient(loss, policy_network.trainable_variables + value_network.trainable_variables)
                optimizer.apply_gradients(zip(gradient_policy,policy_network.trainable_variables + value_network.trainable_variables))
